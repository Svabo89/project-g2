...
public class EdgeFileConverter extends FileParser {
    ...
    while ((currentLine = bufferedReader.readLine()) != null) {
      ...
      if (currentLine.startsWith("Figure ")) { // this is the start of a Figure entry
        ...
        if (!currentLine.startsWith("Style")) { // this is to weed out other Figures, like Labels
          ...
          style = currentLine.substring(
              currentLine.indexOf("\"") + 1,
              currentLine.lastIndexOf("\"")); // get the Style parameter
      ...
      if (currentLine.startsWith("Connector ")) { // this is the start of a Connector entry
        ...
        endStyle1 = currentLine.substring(
            currentLine.indexOf("\"") + 1,
            currentLine.lastIndexOf("\"")); // get the End1 parameter
        currentLine = bufferedReader.readLine().trim(); // End2
        endStyle2 = currentLine.substring(
            currentLine.indexOf("\"") + 1,
            currentLine.lastIndexOf("\"")); // get the End2 parameter
  ...
  public void resolveConnectors() {
    ...
    for (int cIndex = 0; cIndex < connectors.length; cIndex++) {
      ...
      if (connectors[cIndex].isEP1Field() && connectors[cIndex].isEP2Field()) { // both endpoints are fields, implies
        ...
        // ConverterGUI.setReadSuccess(false); //this tells GUI not to populate JList
        // components
        log.warn(
            "The file \"" +
                parseFile +
